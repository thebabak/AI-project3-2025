...
# Output dirs
# ------------------------------
BASE_DIR = os.path.join("logs", "main3-5btensorboard+frac1to5")
...
def evaluate_model_with_metrics(model, loader, criterion, subcats, device, fold, epoch, model_type,
    ...
    if writer is not None:
        ...
        sns.heatmap(cm, annot=False, fmt='d', cmap='Blues',
                    ...
                    yticklabels=subcats[:cm.shape[0]]); plt.title(f'Confusion Matrix - {model_type} ({split_type})')
        plt.xlabel('Predicted'); plt.ylabel('True')
...
# --- New: compare ALL approaches without removing your original one ---
...
# New: fold comparison including PEFT + Fractional
...
if __name__ == "__main__":
    ...
    fractional_fold = init_fold_metrics()  # NEW

    timing = {"Full Fine-Tuning":[], "Text Encoder Fine-Tuning":[],
              "Vision-Only Fine-Tuning":[], "Partial Fine-Tuning":[], "LoRA Adapters":[],
              "PEFT LoRA": [], "Fractional (1-5%)": []}  # NEW key

    # store param stats per approach/fold
    params_info = []  # rows appended per approach-run
    ...
    # ---- Harden checkpoint compatibility (avoids KeyErrors) ----
    ...
    # helper to create TensorBoard writers
    def tb(log_dir):
        return SummaryWriter(log_dir=os.path.join(log_dir, "tb"))
    ...
    for fold, (train_val_idx, test_idx) in enumerate(kf.split(dataset)):
        ...
        if "full" not in checkpoint["completed_approaches"][str(fold)]:
            ...
            met = train_model(model, train_loader, val_loader, test_loader, crit, opt, 4,
                              subcategories, device, log_dir, fold+1, 4, 2, "Full Fine-Tuning", writer=writer)
            evaluate_model_with_metrics(model, val_loader, crit, subcategories, device, fold+1, 999,
                                        "Full Fine-Tuning", "Validation", writer=writer)
            evaluate_model_with_metrics(model, test_loader, crit, subcategories, device, fold+1, 999,
        ...
        if "text" not in checkpoint["completed_approaches"][str(fold)]:
            ...
            met = train_model(model, train_loader, val_loader, test_loader, crit, opt, 4,
                              subcategories, device, log_dir, fold+1, 4, 2, "Text Encoder Fine-Tuning", writer=writer)
            evaluate_model_with_metrics(model, val_loader, crit, subcategories, device, fold+1, 999,
        ...
        if "vision" not in checkpoint["completed_approaches"][str(fold)]:
            ...
            met = train_model(model, train_loader, val_loader, test_loader, crit, opt, 4,
                              subcategories, device, log_dir, fold+1, 4, 2, "Vision-Only Fine-Tuning", writer=writer)
            evaluate_model_with_metrics(model, val_loader, crit, subcategories, device, fold+1, 999,
        ...
        if "partial" not in checkpoint["completed_approaches"][str(fold)]:
            ...
            met = train_model(model, train_loader, val_loader, test_loader, crit, opt, 4,
                              subcategories, device, log_dir, fold+1, 4, 2, "Partial Fine-Tuning", writer=writer)
            evaluate_model_with_metrics(model, val_loader, crit, subcategories, device, fold+1, 999,
        ...
        if "lora" not in checkpoint["completed_approaches"][str(fold)]:
            ...
            met = train_model(model, train_loader, val_loader, test_loader, crit, opt, 4,
                              subcategories, device, log_dir, fold+1, 4, 2, "LoRA Adapters", writer=writer)
            evaluate_model_with_metrics(model, val_loader, crit, subcategories, device, fold+1, 999,
        ...
        if "peft" not in checkpoint["completed_approaches"][str(fold)]:
            ...
            met = train_model(peft_model, train_loader_peft, val_loader_peft, test_loader_peft, crit, opt, 4,
                              ...
                              accumulation_steps=4, validate_every=1, model_type="PEFT LoRA", writer=writer)
            evaluate_model_with_metrics(peft_model, val_loader_peft, crit, subcategories, device, fold+1, 999,
                                        "PEFT LoRA","Validation", writer=writer)
            evaluate_model_with_metrics(peft_model, test_loader_peft, crit, subcategories, device, fold+1, 999,
        ...
        if "fractional" not in checkpoint["completed_approaches"][str(fold)]:
            ...
            met = train_model(frac_model, train_loader, val_loader, test_loader, crit, opt, 4,
                              ...
                              model_type="Fractional (1-5%)", writer=writer)
            evaluate_model_with_metrics(frac_model, val_loader, crit, subcategories, device, fold+1, 999,
                                        "Fractional (1-5%)","Validation", writer=writer)
            evaluate_model_with_metrics(frac_model, test_loader, crit, subcategories, device, fold+1, 999,
            ...
            v = evaluate_model_with_metrics(frac_model, val_loader, crit, subcategories, device, fold+1, 999,
                                            "Fractional (1-5%)","Validation")
            t = evaluate_model_with_metrics(frac_model, test_loader, crit, subcategories, device, fold+1, 999,
                                            "Fractional (1-5%)","Test")
            ...
            save_logs_as_zip(log_dir, f"fractional_finetune_fold_{fold+1}_logs")
    ...
    # Fold comparison tables (old + all)
    ...
    # Param stats CSV (efficiency) + dedicated LoRA vs Full slice
    save_csv(pd.DataFrame(params_info), "params_info.csv")
    # LoRA vs Full comparison per fold using timing & param share
    lora_vs_full_rows=[]
    for i in range(1, k_folds+1):
        # get last recorded stats per fold
        def pick(approach):
            rows=[r for r in params_info if r["Fold"]==i and r["Approach"]==approach]
            return rows[-1] if rows else None
        full_r = pick("Full Fine-Tuning")
        lora_r = pick("LoRA Adapters")
        if full_r or lora_r:
            lora_vs_full_rows.append({
                "Fold": i,
                "Full Trainable %": (full_r["TrainablePct"]*100 if full_r else np.nan),
                "LoRA Trainable %": (lora_r["TrainablePct"]*100 if lora_r else np.nan),
                "Full Trainable Params": (full_r["TrainableParams"] if full_r else np.nan),
                "LoRA Trainable Params": (lora_r["TrainableParams"] if lora_r else np.nan),
                "Full Duration (s)": (timing["Full Fine-Tuning"][i-1] if len(timing["Full Fine-Tuning"])>=i else np.nan),
                "LoRA Duration (s)": (timing["LoRA Adapters"][i-1] if len(timing["LoRA Adapters"])>=i else np.nan),
            })
    save_csv(pd.DataFrame(lora_vs_full_rows), "lora_vs_full_efficiency.csv")
