...
class HfCLIPWithPEFT(nn.Module):
    ...
    To avoid the 'inputs_embeds' kwarg injection, we call the text peft_model.base_model(...)
    directly and then do pooling + projection ourselves.
    """
    def __init__(self, model_name, device, subcategories,
        ...
        # (Optional) LoRA on VISION tower – disabled by default due to PEFT kwarg leakage on some versions
        if lora_on_vision:
    ...
    def _encode_images(self, pixel_values: torch.Tensor) -> torch.Tensor:
        # Use standard path (vision usually NOT wrapped)
    ...
    def _encode_text_with_peft(self, input_ids: torch.Tensor, attention_mask: torch.Tensor = None,
        ...
        """
        Bypass PeftModel.forward by calling .base_model(...) directly.
        This still uses LoRA layers (they were injected into base_model).
        """
        text_peft = self.clip.text_model
        # Preferred: use base_model if present
        text_core = getattr(text_peft, "base_model", text_peft)

        # Forward through CLIPTextTransformer -> returns BaseModelOutputWithPooling
        # ...existing code...
        outputs = text_core(
                input_ids=input_ids,
...
def build_text_inputs(model, subcats, device):
    ...
    try:
        ...
        if isinstance(enc, dict):
            # filter to keys the CLIP text model accepts
            allowed = {"input_ids", "attention_mask", "position_ids"}
...
def compare_metrics(full_m, text_m, vision_m, partial_m, lora_m, k_folds):
    ...
    panels = [("Validation Loss","val_loss"), ("Test Loss","test_loss"),
              ("Validation Acc","val_acc"), ("Test Acc","test_acc"), ("Validation F1","f1_val")]
...
if __name__ == "__main__":
    ...
    num_workers = min(8, os.cpu_count()); print("Workers (OpenCLIP):", num_workers)
    ...
    for fold, (train_val_idx, test_idx) in enumerate(kf.split(dataset)):
        ...
        if "peft" not in checkpoint["completed_approaches"][str(fold)]:
            print("\n[6/6] PEFT LoRA (HF transformers CLIP, text-only) ...")
            start=time.time()
            ...
            train_loader_peft = DataLoader(train_raw, batch_size=32, shuffle=True,
                                           num_workers=0, pin_memory=True, collate_fn=peft_collate)
