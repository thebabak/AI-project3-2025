...
# >>> NEW: a raw-PIL dataset (for HF CLIP + CLIPProcessor)
...
def make_clip_collate_fn(processor, device):
    def _fn(batch):
...
# >>> NEW: True PEFT LoRA on Hugging Face CLIP
class HfCLIPWithPEFT(nn.Module):
    """
    transformers' CLIP + PEFT LoRA on both encoders.
    Uses CLIP similarity vs class prompts to produce class logits.
    """
    def __init__(self, model_name, device, subcategories,
        ...
        # Expose tokenizer for your existing train/eval helpers
        self.tokenizer = self.processor.tokenizer
        self.class_prompts = [f"a photo of {c}" for c in subcategories]

        # LoRA config & injection for both encoders
    ...
    def forward(self, images, text_inputs=None):
        # 'images' here must be CLIP pixel_values (B,3,224,224) from CLIPProcessor
...
def evaluate_model_with_metrics(model, loader, criterion, subcats, device, fold, epoch, model_type, split_type="Validation"):
    ...
    if hasattr(model, "tokenizer"):
        texts = model.tokenizer([f"a photo of {c}" for c in subcats], padding=True, return_tensors="pt").to(device)
...
if __name__ == "__main__":
    ...
    peft_fold   = init_fold_metrics()   # <<< NEW
    ...
    for fold, (train_val_idx, test_idx) in enumerate(kf.split(dataset)):
        ...
        # >>> NEW: 6) Hugging Face PEFT LoRA on transformers CLIP
        if "peft" not in checkpoint["completed_approaches"][str(fold)]:
            ...
            collate = make_clip_collate_fn(peft_model.processor, device)
            train_loader_peft = DataLoader(train_raw, batch_size=32, shuffle=True,  num_workers=num_workers, pin_memory=True, collate_fn=collate)
            val_loader_peft   = DataLoader(val_raw,   batch_size=32, shuffle=False, num_workers=num_workers, pin_memory=True, collate_fn=collate)
            test_loader_peft  = DataLoader(test_raw,  batch_size=32, shuffle=False, num_workers=num_workers, pin_memory=True, collate_fn=collate)

            crit = nn.CrossEntropyLoss(weight=class_weights)
            params = [p for p in peft_model.parameters() if p.requires_grad]  # only LoRA params train
            opt  = optim.AdamW(params, lr=2e-4)
    ...
    for name,d in [("Full",full_fold),("Text",text_fold),("Vision",vision_fold),("Partial",partial_fold),("LoRA",lora_fold),("PEFT",peft_fold)]:
    ...
    plot_fold_wrapper("PEFT LoRA",                peft_fold)  # <<< NEW

    # Comparisons (kept original comparison of 5 to avoid clutter)
    ...
    dump("kfold_peft_lora_metrics",        peft_fold)  # <<< NEW
