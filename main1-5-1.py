# %% [markdown]
## Image Classification with ConvNeXt: Full Workflow with Manual Weight Loading, GPU Utilization, Batch Visualization, and Confusion Matrix
...
# Replace "path/to/open_clip_pytorch_model.bin" with the actual path to your local weights
...
# %% [markdown]
### 7. Visualize Batch Samples

def visualize_batch_samples(loader, subcategories, title="Batch Samples"):
    """Visualize a batch of samples from a DataLoader, with denormalization"""
    ...
    images, labels = batch
    images, labels = images.cpu(), labels.cpu()  # Move to CPU for visualization (if on GPU)

    # Denormalize the images
    denormalized_images = images.clone()
    for i in range(3):  # Assuming images are RGB (C=3)
        denormalized_images[:, i, :, :] = images[:, i, :, :] * std[i] + mean[i]

    # Plot the batch
    plt.figure(figsize=(15, 5))
    for i in range(min(len(images), 10)):  # Display up to 10 images
        plt.subplot(1, 10, i + 1)
        plt.imshow(denormalized_images[i].permute(1, 2, 0).numpy())  # Convert tensor to HWC format
        plt.title(subcategories[labels[i]])
        plt.axis('off')
...
# %% [markdown]
### 8. Select and Describe Sample Images

def select_and_describe_images(dataset, subcategories, num_samples=8):
    """
    Select 8 sample images from the dataset and generate custom textual descriptions.
    Args:
        dataset: The dataset containing images and labels.
        subcategories: List of subcategory names corresponding to labels.
        num_samples: Number of sample images to select and describe (default is 8).
    """
    # Randomly select 8 samples from the dataset
    selected_indices = [int(i) for i in np.random.choice(len(dataset), num_samples, replace=False)]  # Convert indices to Python int
    selected_samples = [dataset[i] for i in selected_indices]

    # Display the images and their custom descriptions
    plt.figure(figsize=(15, 10))
    for i, sample in enumerate(selected_samples):
        image = sample['image']
        label = subcategories.index(sample['subCategory'])
        description = f"This is a {subcategories[label]}, suitable for various occasions."
    ...
    plt.suptitle("Sample Images with Custom Descriptions", fontsize=16)
    plt.tight_layout()
    plt.show()


# Call the function to describe and display sample images
select_and_describe_images(dataset, subcategories, num_samples=8)
...
def k_fold_cross_validation(model, dataset, subcategories, k_folds=5, batch_size=32, num_epochs=3, lr=1e-4):
    ...
    for fold, (train_idx, val_idx) in enumerate(kfold.split(dataset)):
        ...
        # Visualize batch samples for this fold
        visualize_batch_samples(train_loader, subcategories, title=f"Training Batch Samples (Fold {fold + 1})")
...
results = k_fold_cross_validation(convnext_model, dataset, subcategories, k_folds=5, batch_size=32, num_epochs=3,
                                  lr=1e-4)