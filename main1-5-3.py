# %% [markdown]
## Image Classification with ConvNeXt (as CLIPFineTuner): Full Workflow with Manual Weight Loading, GPU Utilization, Batch Visualization, Confusion Matrix, and Cosine Similarity
...
# Directory for saving all logs, images, metrics, and model checkpoints
LOG_DIR = "logs/1-5-3/"
...
convnext_model, preprocess_train, preprocess_val = load_and_print_convnext_model(
    ...
    local_weights_path=None  # Set this if you want to load manually
)
...
# %% [markdown]
### 7. Fine-Tuning ConvNeXt Model (renamed as CLIPFineTuner)
...
# %% [markdown]
### 9. Select Images and Compute Similarity

def select_images_and_compute_similarity(dataset, subcategories, num_samples=8):
    """Select random images, generate descriptions, compute similarity, and visualize matrix."""
    ...
    descriptions = [f"This is a {sample['subCategory']}, suitable for various occasions."
    ...
    sns.heatmap(similarities, annot=True, cmap='viridis', fmt='.2f',
                xticklabels=[f"Desc {i+1}" for i in range(num_samples)],
                yticklabels=[f"Image {i+1}" for i in range(num_samples)])
    plt.title("Cosine Similarity Matrix")
    plt.xlabel("Descriptions")
    plt.ylabel("Images")
    plt.savefig(os.path.join(LOG_DIR, "cosine_similarity_matrix.png"))
    plt.show()
    plt.close()

    # Display individual images with descriptions and diagonal similarity scores
    for i, (image, description) in enumerate(zip(images, descriptions)):
        ...
        plt.savefig(os.path.join(LOG_DIR, f"cosine_similarity_image_{i+1}.png"))
        plt.show()
        plt.close()
...
def k_fold_cross_validation(model, dataset, subcategories, k_folds=5, batch_size=32, num_epochs=3, lr=1e-4):
    ...
    for fold, (train_idx, val_idx) in enumerate(kfold.split(dataset)):
        ...
        # Save per-fold accuracy
        with open(os.path.join(LOG_DIR, "kfold_accuracies.txt"), "a") as f:
            f.write(f"Fold {fold + 1} Validation Accuracy: {val_acc:.4f}\n")

    avg_acc = sum(results.values()) / k_folds
    print(f"\nAverage Validation Accuracy across {k_folds} folds: {avg_acc:.2f}%")
    with open(os.path.join(LOG_DIR, "kfold_accuracies.txt"), "a") as f:
        f.write(f"Average Validation Accuracy across {k_folds} folds: {avg_acc:.4f}\n")
    return results
...
# Compute cosine similarity and visualize similarity matrix
select_images_and_compute_similarity(dataset, subcategories, num_samples=8)