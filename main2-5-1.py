...
# %% [markdown]
### 3. Custom CLIP Model with Variable Linear Layers

class CustomCLIPFineTuner(nn.Module):
    """
    Fine-tuning wrapper for CLIP's visual encoder with a customizable classifier.
    Supports variable numbers of linear layers (1, 2, or 3).
    """
    ...
    def __init__(self, base_model, num_classes, num_layers=1):
        ...
        # Freeze all parameters in the visual encoder
        for param in self.visual_encoder.parameters():
            param.requires_grad = False

        # Determine the output size dynamically
        dummy_input = torch.randn(1, 3, 224, 224).to(next(base_model.parameters()).device)  # Match device
        with torch.no_grad():
            output_size = self.visual_encoder(dummy_input).shape[1]  # Get feature dimension

        # Replace the last projection layer with a custom classifier
        layers = []

        # Build a classifier with the specified number of layers
        for i in range(num_layers):
            layers.append(nn.Linear(output_size, output_size if i < num_layers - 1 else num_classes))
            if i < num_layers - 1:  # Add activation between layers
    ...
    def forward(self, images):
        ...
        # Pass visual encoder features through the custom classifier
...
def train_model(model, train_loader, val_loader, num_epochs=3, lr=1e-4):
    ...
    for epoch in range(num_epochs):
        ...
        for images, labels in progress_bar:
            ...
            with torch.amp.autocast(device_type='cuda' if device.type == 'cuda' else 'cpu'):  # Correct usage
        ...
        if val_acc > best_acc:
            best_acc = val_acc
            torch.save(model.state_dict(), f'best_clip_model_{len(model.classifier)}_layers.pth')
...
# %% [markdown]
### 9. K-Fold Cross-Validation and Comparison

def train_and_compare_models(dataset, subcategories, clip_model, k_folds=5, batch_size=32, num_epochs=3, lr=1e-4):
    """Train models with 1, 2, and 3 linear layers and compare performance."""
    results = {}
    num_layers_list = [1, 2, 3]

    for num_layers in num_layers_list:
        print(f"\n--- Training Model with {num_layers} Linear Layer(s) ---")
        ...
        for fold, (train_idx, val_idx) in enumerate(kfold.split(dataset)):
            ...
            model = CustomCLIPFineTuner(clip_model, len(subcategories), num_layers=num_layers).to(device)
        ...
        avg_acc = sum(fold_results.values()) / k_folds
        print(f"\nAverage Validation Accuracy with {num_layers} Linear Layer(s): {avg_acc:.2f}%")
        results[num_layers] = avg_acc

    # Compare results
    print("\n--- Model Comparison ---")
    for num_layers, avg_acc in results.items():
        print(f"Model with {num_layers} Linear Layer(s): Average Validation Accuracy = {avg_acc:.2f}%")
...
if __name__ == '__main__':
    ...
    # Train and compare models with 1, 2, and 3 linear layers
    results = train_and_compare_models(dataset, subcategories, clip_model, k_folds=5, batch_size=32, num_epochs=3, lr=1e-4)