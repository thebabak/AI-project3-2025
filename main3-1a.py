...
if __name__ == '__main__':
    ...
    for fold, (train_indices, val_indices) in enumerate(kf.split(dataset)):
        ...
        # Model, optimizer, criterion (new for each fold!)
        model = FullFineTunedCLIP(clip_model, len(subcategories), num_layers=1).to(device)
        criterion = nn.CrossEntropyLoss(weight=class_weights)
        optimizer = optim.AdamW(model.parameters(), lr=1e-4)

        fold_log_dir = f"logs/3-1/kfold_finetune/fold_{fold+1}/"
        ...
        # Load best model for this fold for final val evaluation
        ...
        save_logs_as_zip(fold_log_dir, f"logs/3-1/kfold_finetune/fold_{fold+1}_logs")
