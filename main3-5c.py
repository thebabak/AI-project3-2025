# -*- coding: utf-8 -*-
import os, json, time, shutil, warnings, platform
...
class CLIPCollate:
    ...
    def __call__(self, batch):
        images, labels = zip(*batch)  # images are PIL
        enc = self.processor(images=list(images), return_tensors="pt")
        pixel_values = enc["pixel_values"]  # CPU tensor; training loop moves to device
...
class PartialFineTunedCLIP(nn.Module):
    ...
    def _freeze_layers(self, module, freeze_percentage, enc_name):
        ...
        cutoff = int(total * freeze_percentage)
...
class HfCLIPWithPEFT(nn.Module):
    def __init__(self, model_name, device, subcategories,
                 lora_r=8, lora_alpha=16, lora_dropout=0.05, freeze_non_lora=True):
        super().__init__()
        ...
        self.clip.vision_model = get_peft_model(self.clip.vision_model, vision_config)
        self.clip.text_model = get_peft_model(self.clip.text_model, text_config)
    ...
    def forward(self, images, text_inputs=None):
        ...
        image_features = vision_outputs.pooler_output if hasattr(vision_outputs, 'pooler_output') else vision_outputs[1]

        # Process text
        if text_inputs is None:
            text_inputs = self.tokenizer(self.class_prompts, padding=True, return_tensors="pt").to(self.device)
        ...
        text_features = text_outputs.pooler_output if hasattr(text_outputs, 'pooler_output') else text_outputs[1]

        # Project features
        image_embeds = self.clip.visual_projection(image_features)
...
def build_class_text_inputs(model, subcats, device):
    tok = getattr(model, "tokenizer", None)
    if tok is None:
        return None
    prompts = [f"a photo of {c}" for c in subcats]

    if hasattr(tok, 'pad'):  # HF tokenizer
        enc = tok(prompts, padding=True, return_tensors="pt")
        if hasattr(enc, "to"):
            enc = enc.to(device)
        return enc
    else:  # OpenCLIP tokenizer
        tokens = tok(prompts)
...
def evaluate_model_with_metrics(model, loader, criterion, subcats, device, fold, epoch, model_type,
    ...
    texts = build_class_text_inputs(model, subcats, device)
...
def compare_metrics(full_m, text_m, vision_m, partial_m, lora_m, peft_m, k_folds):
    def s(d, k):
        return d.get(k, [])

    max_len = max(len(s(full_m, "val_loss")), len(s(text_m, "val_loss")),
                  len(s(vision_m, "val_loss")), len(s(partial_m, "val_loss")),
                  len(s(lora_m, "val_loss")), len(s(peft_m, "val_loss")), k_folds)
...
def plot_full_vs_partial_vs_lora(full_m, partial_m, lora_m, peft_m, k_folds):
    ...
    for i, (title, key) in enumerate(pairs, 1):
        ...
        plt.plot(folds[:len(s(peft_m, key))], s(peft_m, key), marker="*", label="PEFT LoRA")
        plt.xlabel("Fold")
        plt.ylabel(title)
        plt.title(f"Full vs Partial vs LoRA — {title}")
        plt.legend()
    save_png("full_vs_partial_vs_lora.png")


def create_fold_comparison_table(full_m, text_m, vision_m, partial_m, lora_m, peft_m, k_folds):
    def get(m, k, i):
        v = m.get(k, [])
    ...
    for i in range(k_folds):
        rows.append({
            ...
            "PEFT Train Loss": get(peft_m, "train_loss", i), "PEFT Train Acc": get(peft_m, "train_acc", i),
            "PEFT Val Loss": get(peft_m, "val_loss", i), "PEFT Val Acc": get(peft_m, "val_acc", i),
            "PEFT Test Loss": get(peft_m, "test_loss", i), "PEFT Test Acc": get(peft_m, "test_acc", i),
        })
    ...
    rows.append({
        ...
        "PEFT Train Loss": avg(peft_m, "train_loss"), "PEFT Train Acc": avg(peft_m, "train_acc"),
        "PEFT Val Loss": avg(peft_m, "val_loss"), "PEFT Val Acc": avg(peft_m, "val_acc"),
        "PEFT Test Loss": avg(peft_m, "test_loss"), "PEFT Test Acc": avg(peft_m, "test_acc"),
    })
...
class LoRAClipBothEncoders(nn.Module):
    """
    Custom LoRA implementation for both vision and text encoders of CLIP
    without using PEFT library.
    """
    ...
    def __init__(self, base_model, subcategories, device, lora_r=8, lora_alpha=16, lora_dropout=0.05):
        ...
        # Store original model components
        ...
        vis_targets = self._discover_lora_targets(self.visual)
        txt_targets = self._discover_lora_targets(self.text_encoder)

        print(f"[LoRA] Visual targets substrings: {vis_targets}")
        print(f"[LoRA] Text targets substrings: {txt_targets}")

        # Inject LoRA modules
        self._inject_lora_modules(self.visual, vis_targets, lora_r, lora_alpha, lora_dropout)
        self._inject_lora_modules(self.text_encoder, txt_targets, lora_r, lora_alpha, lora_dropout)
    ...
    def _discover_lora_targets(self, model, prefer=("qkv", "q_proj", "k_proj", "v_proj",
                                                    "out_proj", "proj", "fc1", "fc2",
                                                    "in_proj", "mlp", "attn")):
        """Discover linear layers in model that match preferred target names"""
        linear_names = []
    ...
    def _inject_lora_modules(self, root, target_substrings, r=8, alpha=16, dropout=0.05):
        """Replace linear layers with LoRA wrappers"""
        for name, module in list(root.named_children()):
            ...
            if isinstance(full, nn.Linear) and any(s in name.lower() for s in target_substrings):
                ...
                self._inject_lora_modules(full, target_substrings, r=r, alpha=alpha, dropout=dropout)

    def encode_text(self, token_ids):
        """Encode text using the LoRA-modified text encoder"""
        x = self.token_embedding(token_ids)
        x = x + self.positional_embedding
        x = x.permute(1, 0, 2)
        x = self.text_encoder(x)
    ...
    def forward(self, images, text_inputs=None):
        """Forward pass using LoRA-modified encoders"""
...
if __name__ == "__main__":
    ...
    if os.path.exists(ckpt_path):
        try:
            ...
            for k in ["full_fold", "text_fold", "vision_fold", "partial_fold", "lora_fold", "peft_fold", "timing"]:
                if k in checkpoint:
    ...
    for fold, (train_val_idx, test_idx) in enumerate(kf.split(dataset)):
        ...
        if "full" not in checkpoint["completed_approaches"][str(fold)]:
            ...
            full_fold["val_loss"].append(v[0])
            full_fold["val_acc"].append(v[1])
            full_fold["test_loss"].append(t[0])
            full_fold["test_acc"].append(t[1])
            full_fold["precision"].append(v[2])
            full_fold["recall"].append(v[3])
            full_fold["f1"].append(v[4])
        ...
        if "text" not in checkpoint["completed_approaches"][str(fold)]:
            ...
            text_fold["val_loss"].append(v[0])
            text_fold["val_acc"].append(v[1])
            text_fold["test_loss"].append(t[0])
            text_fold["test_acc"].append(t[1])
            text_fold["precision"].append(v[2])
            text_fold["recall"].append(v[3])
            text_fold["f1"].append(v[4])
        ...
        if "vision" not in checkpoint["completed_approaches"][str(fold)]:
            ...
            vision_fold["val_loss"].append(v[0])
            vision_fold["val_acc"].append(v[1])
            vision_fold["test_loss"].append(t[0])
            vision_fold["test_acc"].append(t[1])
            vision_fold["precision"].append(v[2])
            vision_fold["recall"].append(v[3])
            vision_fold["f1"].append(v[4])
        ...
        if "partial" not in checkpoint["completed_approaches"][str(fold)]:
            print("\n[4/6] Partial Fine-Tuning (last 30% both encoders)")
            start = time.time()
            ...
            partial_fold["val_loss"].append(v[0])
            partial_fold["val_acc"].append(v[1])
            partial_fold["test_loss"].append(t[0])
            partial_fold["test_acc"].append(t[1])
            partial_fold["precision"].append(v[2])
            partial_fold["recall"].append(v[3])
            partial_fold["f1"].append(v[4])
        ...
        if "lora" not in checkpoint["completed_approaches"][str(fold)]:
            print("\n[5/6] LoRA Adapters (custom; both encoders) ...")
            start = time.time()
            ...
            lora_fold["val_loss"].append(v[0])
            lora_fold["val_acc"].append(v[1])
            lora_fold["test_loss"].append(t[0])
            lora_fold["test_acc"].append(t[1])
            lora_fold["precision"].append(v[2])
            lora_fold["recall"].append(v[3])
            lora_fold["f1"].append(v[4])
        ...
        # 6) Hugging Face PEFT LoRA on transformers CLIP
        if "peft" not in checkpoint["completed_approaches"][str(fold)]:
            print("\n[6/6] PEFT LoRA (HF transformers CLIP) ...")
            start = time.time()

            # Raw-PIL datasets + CLIPProcessor-based collate for correct pixel_values
            ...
            collate = CLIPCollate(peft_model.processor)

            # Windows tip: keep workers=0 to avoid additional pickling pitfalls
            peft_workers = 0 if platform.system().lower().startswith("win") else num_workers

            train_loader_peft = DataLoader(train_raw, batch_size=32, shuffle=True,
                                           num_workers=peft_workers, pin_memory=True, collate_fn=collate)
            ...
            crit = nn.CrossEntropyLoss(weight=class_weights)
            params = [p for p in peft_model.parameters() if p.requires_grad]  # only LoRA params
            opt = optim.AdamW(params, lr=2e-4)
            ...
            met = train_model(peft_model, train_loader_peft, val_loader_peft, test_loader_peft, crit, opt, 4,
                              subcategories, device, log_dir, fold + 1, 4, 1, "PEFT LoRA")
            ...
            v = evaluate_model_with_metrics(peft_model, val_loader_peft, crit, subcategories, device, fold + 1, 999,
                                            "PEFT LoRA", "Validation")
            t = evaluate_model_with_metrics(peft_model, test_loader_peft, crit, subcategories, device, fold + 1, 999,
                                            "PEFT LoRA", "Test")
            ...
            peft_fold["val_loss"].append(v[0])
            peft_fold["val_acc"].append(v[1])
            peft_fold["test_loss"].append(t[0])
            peft_fold["test_acc"].append(t[1])
            peft_fold["precision"].append(v[2])
            peft_fold["recall"].append(v[3])
            peft_fold["f1"].append(v[4])

            save_logs_as_zip(log_dir, f"peft_lora_fold_{fold + 1}_logs")
    ...
    # Comparisons
    compare_metrics(full_fold, text_fold, vision_fold, partial_fold, lora_fold, peft_fold, k_folds)
    plot_full_vs_partial_vs_lora(full_fold, partial_fold, lora_fold, peft_fold, k_folds)


    # CSV dumps
    def dump(name, d):
        save_csv(d, f"{name}.csv")
    ...
    create_fold_comparison_table(full_fold, text_fold, vision_fold, partial_fold, lora_fold, peft_fold, k_folds)
    ...
    for approach, durations in timing.items():
        if durations:
            ...
            for i in range(1, k_folds + 1):
                timing_rows.append(
                    {"Approach": approach, "Fold": i, "Duration (seconds)": 0, "Formatted Duration": "N/A"})
    save_csv(pd.DataFrame(timing_rows), "execution_timing.csv")

    overall_end = time.time()
    h, rem = divmod(overall_end - overall_start, 3600)
    m, s = divmod(rem, 60)
