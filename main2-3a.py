...
def train_model_with_tensorboard(
    ...
    """
    Train the model and log training and validation losses to TensorBoard.
    """
    criterion = nn.CrossEntropyLoss(weight=class_weights.to(device))  # Move class weights to GPU
    ...
    writer = SummaryWriter(log_dir=f"{log_dir}{optimizer_type}_fold_{fold_index}/")  # Separate logs for each optimizer
    best_acc = 0.0
...
### Main Workflow with Separate Optimizer Folds
...
if __name__ == '__main__':
    ...
    for optimizer_type in ["adam", "adamw"]:  # Loop through both optimizers
