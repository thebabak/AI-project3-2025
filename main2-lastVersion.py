...
class CustomCLIPFineTuner(nn.Module):
    def __init__(self, base_model, num_classes, num_layers=1):
        ...
        # Optionally unfreeze selected blocks/heads (tweak for your model depth)
        for name, p in self.visual_encoder.named_parameters():
...
# -----------------------------
# Plotting & CSV (save to png/ and csv/)
# -----------------------------
def plot_metrics(metrics, k_folds, out_dir, title_suffix=""):
    ...
    plt.tight_layout()
    out_path = os.path.join(out_dir, f"metrics_plot{title_suffix}.png")
...
if __name__ == "__main__":
    ...
    # Prepare output folders for artifacts
    ...
    for optimizer_type in ["adam", "adamw"]:
        for fold_index, (train_idx, val_idx) in enumerate(kf.split(dataset), start=1):
            ...
            # Use CLIP preprocessors to match the backbone exactly
    ...
    # Plots & CSV (save inside png/ and csv/)
