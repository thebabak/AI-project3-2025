...
def evaluate_model_with_metrics(model, loader, criterion, subcategories, device, display_predictions=False):
    model.eval()
    total_loss = 0.0
    true_labels = []
    predictions = []
...
def plot_fold_metrics(metrics, k_folds):
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig("fold_metrics.png")  # Save plot for documentation
    plt.close()
...
def train_full_finetune(
    ...
    for epoch in range(num_epochs):
        ...
        correct_train = 0
        total_train = 0
        optimizer.zero_grad()  # Reset gradients at start of epoch for accumulation
        ...
        if (epoch + 1) % validate_every == 0:
            ...
            # Append dummy values for epochs without validation
            metrics_dict['train_loss'].append(train_loss)
...
if __name__ == '__main__':
    ...
    # Set number of workers based on CPU cores (adjust if needed)
    ...
    for fold, (train_indices, val_indices) in enumerate(kf.split(dataset)):
        ...
        # Optimize DataLoader with higher num_workers and pin_memory
        ...
        # Model with option to freeze encoder, optimizer, criterion
        model = FullFineTunedCLIP(clip_model, len(subcategories), num_layers=1, freeze_encoder=True).to(device)
        criterion = nn.CrossEntropyLoss(weight=class_weights)
        # Optimize only classifier if encoder is frozen, otherwise optimize all parameters
        params_to_optimize = model.classifier.parameters() if model.is_encoder_frozen else model.parameters()
        optimizer = optim.AdamW(params_to_optimize, lr=1e-4)

        fold_log_dir = f"logs/3-1b/kfold_finetune/fold_{fold+1}/"
        metrics_dict = train_full_finetune(
            model, train_loader, val_loader, criterion, optimizer, num_epochs=4,
            subcategories=subcategories, device=device, log_dir=fold_log_dir,
            accumulation_steps=4, validate_every=2  # Gradient accumulation and less frequent validation
        )
        ...
        save_metrics_to_csv({
            ...
            "f1": metrics_dict['f1'],
        }, f"metrics_fold_{fold+1}.csv")

        save_logs_as_zip(fold_log_dir, f"logs/3-1b/kfold_finetune/fold_{fold+1}_logs")
        torch.cuda.empty_cache()  # Clear cache after each fold to manage memory

    # Plot and save overall fold metrics
    plot_fold_metrics(fold_metrics, k_folds)
    save_metrics_to_csv(fold_metrics, "kfold_full_finetune_metrics.csv")
    print("\nK-Fold fine-tuning complete. Fold results saved for analysis.")