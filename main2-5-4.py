...
def visualize_predictions(images_list, subcategories):
    ...
    for i in range(num_samples_to_display):
        ...
        for j in range(min(len(images), num_samples_to_display)):
            ...
            plt.axis("off")
    plt.tight_layout()
    plt.savefig(os.path.join(LOG_DIR, "visualized_predictions.png"))
    plt.show()
...
def train_model_with_tensorboard(
        model, train_loader, val_loader, optimizer_type, num_epochs=3, lr=1e-4, log_dir=LOG_DIR, fold_index=0
):
    ...
    for epoch in range(num_epochs):
        ...
        if f1 > best_acc:
            best_acc = f1
            torch.save(model.state_dict(), os.path.join(LOG_DIR, f'best_clip_model_{optimizer_type}_fold_{fold_index}.pth'))
    writer.close()
...
def plot_metrics(metrics, k_folds):
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(os.path.join(LOG_DIR, "fold_metrics.png"))
    plt.show()
    ...
    for optimizer_type in ["adam", "adamw"]:
        for fold_index, (train_indices, val_indices) in enumerate(kf.split(dataset)):
            ...
            val_loss, precision, recall, f1 = train_model_with_tensorboard(
                model, train_loader, val_loader, optimizer_type=optimizer_type, num_epochs=3, lr=1e-4, log_dir=LOG_DIR,
