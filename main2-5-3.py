...
def initialize_clip_model(device, model_name="ViT-B-32", pretrained_weights="openai"):
    """Initialize CLIP model with preprocessing and tokenizer"""
    try:
        # Create model and transforms with QuickGELU if specified
        model, _, preprocess = open_clip.create_model_and_transforms(
            model_name + "-quickgelu",  # Use QuickGELU model
...
model, preprocess, tokenizer = initialize_clip_model(device, model_name, pretrained_weights)
...
def train_model(model, train_loader, val_loader, num_epochs=10, lr=1e-4):
    ...
    criterion = nn.CrossEntropyLoss()  # No zero_division parameter here
...
def evaluate_model(model, loader):
    ...
    all_labels = []
    total_loss = 0.0
    criterion = nn.CrossEntropyLoss()  # Define criterion for loss calculation
    ...
    avg_loss = total_loss / len(loader)  # Average loss
    return avg_loss, 100 * correct / total  # Return both average loss and accuracy
...
test_loss, test_acc = evaluate_model(model_ft, test_loader)
