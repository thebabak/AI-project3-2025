...
def visualize_predictions(images_list, subcategories):
    ...
    num_samples_to_display = min(8, len(images_list))  # Limit to displaying 8 samples
    plt.figure(figsize=(15, 10))

    for i in range(num_samples_to_display):
        images, labels, predicted = images_list[i]
        for j in range(min(len(images), num_samples_to_display)):  # Display up to 8 images per batch
            plt.subplot(2, 4, j + 1)  # Create a grid of 2 rows and 4 columns
            image = images[j].permute(1, 2, 0).numpy()  # Convert Tensor to NumPy array
            image = (image - image.min()) / (image.max() - image.min())  # Normalize image for display
...
def evaluate_model_with_metrics(model, loader, criterion, subcategories, display_predictions=False):
    ...
    true_labels = []
    predictions = []
    images_list = []  # To store images for visualization
    ...
    with torch.no_grad():
        for images, labels in loader:
            ...
            if display_predictions:  # Collect images for visualization
    ...
    for optimizer_type in ["adam", "adamw"]:
        for fold_index, (train_indices, val_indices) in enumerate(kf.split(dataset)):
            ...
            val_loss, precision, recall, f1 = train_model_with_tensorboard(
                model, train_loader, val_loader, optimizer_type=optimizer_type, num_epochs=3, lr=1e-4, log_dir="logs/",
